...
class TestEntorno(unittest.TestCase):
    def test_eventos(self):
        ...
        self.assertEqual(6, len(ac.cuentas.keys()))
        m = MAE()
        m.cuenta = '4301'
        m.descripcion = "reprueba"
        e.bind(m)
        self.assertEqual(7, len(ac.cuentas.keys()))
        e.unbind(m)
        self.assertEqual(6, len(ac.cuentas.keys()))
    ...
    def test_acumulador(self):
        ...
        e.bind(ap2)
        self.assertEqual(1000, ac.cuentas.get('100')[1].euros_2)
        e.unbind(ap2)
        self.assertEqual(0, ac.cuentas.get('100')[1].euros_2)
        ap3 = crea_apu(haber=1200, fecha=date(2010, 3, 5), cuenta='47700001')
        e.bind(ap3)
        self.assertEqual(-1200, ac.cuentas.get('477')[1].euros_3)
        ap4 = crea_apu(debe=300, fecha=date(2010, 3, 15), cuenta='47700001')
        e.bind(ap4)
        self.assertEqual(-900, ac.cuentas.get('477')[1].euros_3)
        e.unbind(ap3)
        self.assertEqual(300, ac.cuentas.get('477')[1].euros_3)
        self.assertEqual(300, ac.cuentas.get('47700001')[1].euros_3)

        sal = ac.cuentas.get('477')[1]
